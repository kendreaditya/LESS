...
import os
import argparse
from knee_3d_model import Knee3DModel
...
def process_video(video_path, show_windows=True):
    ...
    output_path = os.path.join(os.path.dirname(video_path), f"{name}_output_with_ang_vel_acc_jerk_KNEE_MODEL{ext}")
        ...
        for _ in tqdm(range(total_frames), desc="Processing video"):
            ...
            if results.pose_landmarks:
                ...
                if frame_count >= min_frames:
                    ...
                    knee_angle = np.radians(current_angles['Left Knee Flexion'])
                    knee_velocity = velocities['Left Knee Flexion'][-1] if 'Left Knee Flexion' in velocities else 0
                    
                    t = np.array([0, 1/fps])  # Simulate for one frame duration
                    initial_conditions = [knee_angle, knee_velocity]
                    simulation_results = knee_model.simulate(initial_conditions, t)
                    
                    # Calculate ligament forces
                    acl_force, pcl_force, mcl_force = knee_model.calculate_ligament_forces(simulation_results[-1])
                    acl_forces.append(acl_force)
                    pcl_forces.append(pcl_force)
                    mcl_forces.append(mcl_force)
                    
                    # Display ligament forces on the frame
                    force_text = f"ACL: {acl_force:.2f}, PCL: {pcl_force:.2f}, MCL: {mcl_force:.2f}"
                    print(force_text)
                    cv2.putText(image, force_text, (10, height - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
                    ...
                    if show_windows:
                        ...
                        axs[4].plot(acl_forces, label="ACL Force")
                        axs[5].plot(pcl_forces, label="PCL Force")
                        axs[6].plot(mcl_forces, label="MCL Force")
                        ...
                        axs[4].set_title('ACL Forces')
                        axs[5].set_title('PCL Forces')
                        axs[6].set_title('MCL Forces')
                        plt.pause(0.001)
    ...
    if show_windows:
        ...
        if not os.path.exists('./outputs'):
            os.makedirs('./outputs')
        fig.savefig('./outputs/angles_velocities_accelerations_jerks_KNEE_MODEL.png')
        plt.show()
